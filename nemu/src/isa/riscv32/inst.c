...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , S, {R(rd) = PC + 4; s->dnpc = (src1 + imm) & ~1;});
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, handle_bne); 
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, handle_beqz); 
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, R(rd) = ((int32_t)src1 >= (int32_t)src2)? 1 : 0); // need to 
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, R(rd) = ((int32_t)src1 < (int32_t)src2)? 1 : 0); // need to
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, R(rd) = ((uint32_t)src1 < (uint32_t)src2)? 1 : 0);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, R(rd) = ((uint32_t)src1 >= (uint32_t)src2)? 1 : 0);
