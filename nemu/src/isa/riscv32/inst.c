...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, R(rd) = ((int32_t)src1 >= imm)? 1 : 0); // need to 
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, R(rd) = ((int32_t)src1 < imm)? 1 : 0); // need to
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, R(rd) = ((uint32_t)src1 < imm)? 1 : 0);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, R(rd) = ((uint32_t)src1 >= imm)? 1 : 0);
