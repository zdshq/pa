...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, COM_TO_SOURI(imm, IMMI_SIZE, TO_RD));
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, handle_slti);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, handle_sltiu);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = PC + 4; s->dnpc = (src1 + imm) & ~1;});
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", sb     , S, {R(rd) = PC + 4; s->dnpc = (src1 + (com_to_sour(imm,12) & ~1));});
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, handle_bne); 
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, handle_beqz); 
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, {if((int32_t)src1 > (int32_t)src2) s->dnpc = PC + imm;}); // need to 
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, handle_blt); // need to
