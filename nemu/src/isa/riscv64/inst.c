...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I, uint8_t t = Mr(src1 + src2, 1); R(dest) = SEXT((uint8_t)(t), 8));  //
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, int8_t t = Mr(src1 + src2, 1); R(dest) = SEXT((int8_t)(t), 8)); //
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B, if ((uint32_t)src1 < (uint32_t)src2) s->dnpc = s->pc + dest);  //
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw, R, R(dest) = SEXT(U32(src1) % U32(src2), 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw, R, R(dest) = SEXT(U32(src1) / U32(src2), 32));
